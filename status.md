...
| 2 | Audit error handling so startup and `Init()` failures use exceptions, `Tick()` problems use `Notify()` or related functions, exceptions propagate as high as suitable, and WebUI messages identify the involved module and buffer where applicable while including enough call-chain context to locate the failure. | Pending | Not started | — |
| 3 | Audit header dependencies: remove unnecessary includes while making files include what they directly use instead of relying on transitive includes in most cases. | Pending | Not started | — |
