...
public:
    ...
    ResolveCurrentFilename(std::string & resolved_name)
    {
        ...
        {
            Notify(msg_fatal_error, "Invalid JPEG filename format: " + error, path_);
            return false;
        }
        ...
        {
            Notify(msg_fatal_error, "InputJPEG can only read files from the project directory or UserData.", path_);
            return false;
        }
