...
{
    AudioBufferList * rv = (AudioBufferList *) calloc ( sizeof(AudioBufferList), 1 );
...
{
    AudioBufferList * rv = (AudioBufferList *) calloc ( sizeof(AudioBufferList) + sizeof(AudioBuffer) * channels, 1 );
    unsigned buffer;
...
static unsigned _ABLCompatibleCopy ( const AudioBufferList * src, unsigned srcOffset, AudioBufferList * dst, unsigned dstOffset, unsigned count )
{
    ...
    for ( buffer = 0; buffer < buffersToCopy; buffer++ )
    {
        ...
        srcBytes = (unsigned char *) src->mBuffers[buffer].mData;
        dstBytes = (unsigned char *) dst->mBuffers[buffer].mData;
...
unsigned MTAudioBufferListClear ( AudioBufferList * aList, unsigned offset, unsigned count )
{
    ...
    for ( buffer = 0; buffer < aList->mNumberBuffers; buffer++ )
    {
        ...
        dstBytes = (unsigned char *) aList->mBuffers[buffer].mData;
