...
public:
  ...
  std::string asString(){return std::to_string(v);}
};
...
public:
  ...
  std::string asString() {
      std::string retval="";
      retval += "osc_address: '" + address + "', types: '" + type_tags + "', timetag=" + time_tag.asString() + ", args=[";
      Message::ArgReader arg(*this);
      while (arg.nbArgRemaining() && arg.isOk()) {
        if (arg.isBool()) { bool b; arg.popBool(b); retval += (b?"True":"False"); }
        else if (arg.isInt32()) { int32_t i; arg.popInt32(i); retval+= std::to_string(i); }
        else if (arg.isInt64()) { int64_t h; arg.popInt64(h); retval+= std::to_string( h) + "ll"; }
        else if (arg.isFloat()) { float f; arg.popFloat(f); retval+= std::to_string(f) + "f"; }
        else if (arg.isDouble()) { double d; arg.popDouble(d); retval+= std::to_string(d); }
        else if (arg.isStr()) { std::string s; arg.popStr(s); retval+=  "'" + s + "'"; }
        else if (arg.isBlob()) { std::vector<char> b; arg.popBlob(b); retval+= "Blob " + std::to_string(b.size()) + " bytes"; }
        else {
          assert(0); // I forgot a case..
        }
        if (arg.nbArgRemaining()) retval+= ", ";
      }
      if (!arg.isOk()) { retval+= " ERROR#" + std::to_string((int)arg.getErr()); }
      retval+= "]";
      return retval;
    }  
