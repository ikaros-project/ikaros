...
void UM7::Init()
{
    ...
    s = NULL;
    s = new Serial(GetValue("port"), 115200); // Hardcoded baudrate.

    // Set parameters for the sensor
    // prevent all broadcast from UM7
    Notify(log_level_debug, "Disable broadcast for UM7");
    ...
    Notify(log_level_debug, "Enable ikaros related UM7 broadcast");
    ...
    tx_data[10] = computed_checksum & 0xFF;
    s->SendBytes((char *)tx_data, 11);
    Notify(log_level_debug, "Sending");

    io(Roll_OUTPUT, "ROLL");
    io(Pitch_OUTPUT, "PITCH");
    io(Yaw_OUTPUT, "YAW");
}
    ...
    while (1) // Read data until buffer is empty
    {
        ...
        while (1)
        {
            ...
            if (packet.Address == 0x70) // 112
            {
                ...
                Roll_OUTPUT[0] = estRoll;
                Pitch_OUTPUT[0] = estPitch;
                Yaw_OUTPUT[0] = estYaw;
                // printf("%i\n", estYaw);
            }
            /* code */
...
static InitClass init("UM7", &UM7::Create, "Source/Modules/RobotModules/UM7/");
