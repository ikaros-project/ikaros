...
DynamixelServo::DynamixelServo(DynamixelComm *com, int id, const char * csvPath, int forceModel)
{
	...
	if (!com->ReadMemoryRange(id, protocol, dynamixelMemory, 0, P_CCW_ANGLE_LIMIT))
	{
		printf("Dynamixel: Can not read from id %i, Quitting ikaros.... \n", id);
		std::exit(false);
	}
	
	if (forceModel != 0) // Should only be used if the memoryblock on the servo is corrupt and servo return a not existing model number.
		model = forceModel;
	else
	...
	if (!com->ReadMemoryRange(id, protocol, dynamixelMemory, 0, extraInfo.GetInt("Model Memory"))) 	// Get all memory block
	{
		printf("Dynamixel: Can not read from id %i, Quitting ikaros.... \n", id);
		std::exit(false);
	}
	...
	if (cw == 0 && ccw == 0)
		extraInfo.Set("Operation Type", OP_WHEEL);
	else if (cw == extraInfo.GetInt("Model Position Max") && ccw == extraInfo.GetInt("Model Position Max"))
		extraInfo.Set("Operation Type", OP_MULTI_TURN);
	else
		extraInfo.Set("Operation Type", OP_JOINT);
#ifdef DEBUG_SERVO
...
// MARK: Get formated
float DynamixelServo::GetModelGapAngleFormated(int angleUnit)
{
...
float DynamixelServo::GetGoalPositionFormated(int adress, int angleUnit)
{
	return angle_to_angle(DynamixelToInternal(GetValueAtAdress(adress)), 0, angleUnit);
	//return float (ConvertToAngle(GetValueAtAdress(adress)));
}
...
float DynamixelServo::GetPresentPositionFormated(int adress, int angleUnit)
{
	int posD = GetValueAtAdress(adress); 			// Get dynamixel positon
	float a = DynamixelToInternal(posD); 			// Convert positon to angle
	return angle_to_angle(a, 0, angleUnit); 		// Convert angle to wanted angle
}
...
float DynamixelServo::DynamixelToInternal(int position)
{
	float angleD = float(position)/float(GetModelPositionMax()) * float(GetModelAngleMax()); // Get angle
	float iV = angleD + GetModelGapAngleFormated(0)/2; 	// Convert dynamixel angle MX 0-360 AX 30- 330 (0-300) to real angle
	return iV;
}

int DynamixelServo::InternalToDynamixel(float iV)
{
	float angleD = iV - GetModelGapAngleFormated(0)/2;	// Convert real angle to dynamixel angle MX 0-360 AX 30- 330 (0-300)
	float posD = angleD/float(GetModelAngleMax()); 		// Calculate a value between 0 - 1 0 to MaxAngle
	posD = clip(posD, 0, 1); 							// Clip just to be sure nothing strange is sent to servo. REMOVE?
...
bool DynamixelServo::SetGoalPositionFormated(int adress, float value, int angleUnit)
{
	float iV = angle_to_angle(value, angleUnit, 0); 	// To internal representation (degrees)
	if (iV < GetModelGapAngleFormated(0)/2 || iV > (360-GetModelGapAngleFormated(0)/2))
	{
...
{        
	...
	// Position gap variables for servos without 360 degrees
...
bool DynamixelServo::ReadCSVFileToCtable(CT * controlTable,int model, int * size, const char * csvPath)
{
	...
	if(!fp)
	{
		printf("DynamixelServo: Can not open servo settings file %s",filename.c_str());
		return false;
	}
