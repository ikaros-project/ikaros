...
void DynamixelComm::SyncWriteWithIdRange1(int * servo_id,  unsigned char ** DynamixelMemoeries, int from, int to, int n)
{
    ...
    if (datalength*n+SYNC_WRITE_HEADER_1 > DYNAMIXEL_MAX_BUFFER)
    {
        ...
        int nrServoToSendTo = ((DYNAMIXEL_MAX_BUFFER-SYNC_WRITE_HEADER_1)/datalength)-2;
        ...
        while (nFrom != n)
        {
            ...
            for(int i=nFrom; i<=nTo; i++)
            {
                ...
                memcpy(&outbuf[SYNC_WRITE_HEADER_1 + (datalength+1)*k+1], &DynamixelMemoeries[i][from], (datalength) * sizeof(unsigned char) );
                k++;
            }
            Send1(outbuf);
    ...
    else // No splitting needed.
    {
        ...
        for(int i=0; i<n; i++)
        {
            ...
            memcpy(&outbuf[SYNC_WRITE_HEADER_1 + i*(datalength+1)+1], &DynamixelMemoeries[i][from], datalength * sizeof(unsigned char) );
        }
