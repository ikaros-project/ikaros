...
Epuck::Epuck(Parameter * p): Module(p)
{
    ...
    // add output channels
    AddOutput("PROXIMITY", false, 8);
    AddOutput("ACCELERATION_PLAIN", false, 3);
    AddOutput("ACCELERATION", false, 1);
    AddOutput("ORIENTATION", false, 2);
    AddOutput("INCLINATION", false, 2);
    AddOutput("ENCODER", false, 2);
    AddOutput("MICROPHONE_VOLUME", false, 3);
    AddOutput("MICROPHONE_BUFFER", false, 3, 100);
    AddOutput("MICROPHONE_SCAN_ID", false, 1);
    ...
    {
        AddOutput("RED", false, width, height);
        AddOutput("GREEN", false, width, height);
        AddOutput("BLUE", false, width, height);
    }
}
...
{
	velocity_is_connected = GetInputArray("VELOCITY") != NULL;
	led_is_connected = GetInputArray("LED") != NULL;
	light_is_connected = GetInputArray("LIGHT") != NULL;
