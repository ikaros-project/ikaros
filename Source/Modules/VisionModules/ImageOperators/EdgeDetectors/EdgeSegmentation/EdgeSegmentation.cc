...
Module *
EdgeSegmentation::Create(Parameter * p)
{
    return new EdgeSegmentation(p);
}



EdgeSegmentation::EdgeSegmentation(Parameter * p):
    Module(p)
{
    AddInput("INPUT");
    AddOutput("OUTPUT");
...
void
EdgeSegmentation::CalculateSqrt()
{
...
void
EdgeSegmentation::CalculateAbsDx()
{
    convolve(output, input, dx_kernel, outputsize_x, outputsize_y, 3, 3);
...
void
EdgeSegmentation::CalculateAbsDy()
{
    convolve(output, input, dy_kernel, outputsize_x, outputsize_y, 3, 3);
    abs(output, output, outputsize_x, outputsize_y);
}
