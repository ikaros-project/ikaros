...
MFTracker::calc(const uint8_t* nImage, float marker_size, float ** marker_sizes, int marker_size_ranges, bool use_history = false)
{
    ...
    {
        if(arDetectMarkerLite(const_cast<unsigned char*> (nImage), this->thresh, &marker_info, &marker_num) < 0) // Do not use tracking history
            return;
    }
...
{		 
    ...
    *marker_count = 0;
    ...
    tracker->calc(buffer, marker_size, marker_sizes, marker_size_ranges);
