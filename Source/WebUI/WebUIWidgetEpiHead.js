class WebUIWidgetEpiHead extends WebUIWidgetGraph {
  static template() {
    return [
      ...
      {
        name: "pupilLeftSource",
        default: "",
        type: "source",
        control: "textedit",
      },
      {
        name: "pupilRightSource",
        default: "",
        type: "source",
        control: "textedit",
      },
  ...
  draw() {
    ...
    //this.canvas.translate(offset[0], offset[1]); Make whole head move.
      ...
      this.drawMouth = function (isTopLeds) {
        ...
        for (let i = 0; i < 6; i++) {
          ...
          this.canvas.fillStyle =
            "rgba(" +
            Math.floor(255 * this.mouthColors[sourceIndex][0][i]) +
            "," +
            Math.floor(255 * this.mouthColors[sourceIndex][1][i]) +
            "," +
            Math.floor(255 * this.mouthColors[sourceIndex][2][i]) +
            "," +
            1 +
            ")";
          this.canvas.fill();
    ...
    this.drawEye = function (isRightEye) {
      ...
      this.canvas.arc(
        0,
        0,
        (this.pupil[sourceIndex] / 18) * (17.5 * 0.5625),
        0,
        2 * Math.PI
      );
      this.canvas.fill();
  ...
  update() {
    ...
    this.EyeColors = [
      this.getSource("LeftEyeColor"),
      this.getSource("RightEyeColor"),
    ];
    this.mouthColors = [
    ...
    console.log(this.gaze);
    if (this.gaze.length < 2) this.gaze = [this.gaze[0], this.gaze[0]];

    let defaultPupil = parseFloat(this.parameters.pupilInMM);
    this.pupil = [
      this.getSourceAsFloat("pupilLeftSource", defaultPupil),
      this.getSourceAsFloat("pupilRightSource", defaultPupil),
    ];

    let headPosition = this.getSource("headPosition", [[0, 0]]);
    if (headPosition.length < 2) this.headPosition = [headPosition[0], 0];
